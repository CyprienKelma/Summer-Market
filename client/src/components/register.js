...
function Register() {
    ...
    const handleSignUp = async () => {
        ...
        try {
            ...
            const response = await axios.post('http://10.224.2.12:5001/', {
                username,
                email,
                password,
            });
            ...
            if (response.data.message === 'User created') {
                ...
                console.error('Échec de lenregistrement de lutilisateur');
                // Gérer les erreurs d'enregistrement ici
            }
        } catch (error) {
            console.error('Une erreur sest produite lors de lenregistrement', error);
            // Gérer les erreurs d'enregistrement ici
        }
    };
